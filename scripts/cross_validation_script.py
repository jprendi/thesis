...
class Cross_Validation:
    ...
    def train_k_models(self): #works !
        ...
        """
        directory_path = "trained_models/isotree/kfold_models/"
    ...
    def getnsave_kfold_auc_prauc(self, sigkey):
        ...
        directory_path = "results/isotree/kfold_models/"
        if not os.path.exists(directory_path):
            os.makedirs(directory_path)
        ...
        output_file = "results/isotree/kfold_models/" + f"{self.sigkey}_" + "__".join([f"{key}_{value}" for key, value in self.kwargs.items()]) + "__" + f"{self.k}"
