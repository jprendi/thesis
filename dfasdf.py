...
for sig_key in sigkeys:
    ...
    plt.plot(results_dict_iso["ROCAUC"]["base"], results_dict_iso["ROCAUC"]["mean_curve"], color= 'red', lw=2, label = f'iForest depth (AUC = %.01f%% ± %.01f%%)' % (results_dict_iso["ROCAUC"]["auc_mean"] * 100, results_dict_iso["ROCAUC"]['auc_unc'] * 100))
    
    results = f"results/isotree/reduced/{sig_key}_ntrees_100__scoring_metric_depth__5"
    with open(results, 'rb') as f:
        results_dict_iso = pickle.load(f)
    plt.plot(results_dict_iso["ROCAUC"]["base"], results_dict_iso["ROCAUC"]["mean_curve"], color= 'blue', lw=2, label = f'iForest boxed_ratio (AUC = %.01f%% ± %.01f%%)' % (results_dict_iso["ROCAUC"]["auc_mean"] * 100, results_dict_iso["ROCAUC"]['auc_unc'] * 100))
